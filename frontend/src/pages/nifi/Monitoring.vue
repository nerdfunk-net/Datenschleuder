<template>
  <div class="monitoring-page">
    <div class="page-header">
      ...
      <p class="page-description">Monitor NiFi system diagnostics and health</p>
    </div>
      ...
      <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content modern-modal">
          ...
          <div class="modal-body">
            ...
            <div class="detail-section">
              <h6 class="section-title">
                <i class="pe-7s-graph3"></i> CPU & Threads
              </h6>
              <div class="detail-grid">
                <div class="detail-item">
            ...
            <div class="detail-section">
              ...
              <div v-if="contentRepoStorage" class="storage-subsection">
                ...
                <div class="detail-grid">
                  <div class="detail-item">
                    <span class="detail-key">Identifier</span>
                    <span class="detail-value">{{ contentRepoStorage.identifier }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Utilization</span>
                    <span class="detail-value">{{ contentRepoStorage.utilization }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Used Space</span>
                    <span class="detail-value">{{ contentRepoStorage.used_space }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Free Space</span>
                    <span class="detail-value">{{ contentRepoStorage.free_space }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Total Space</span>
                    <span class="detail-value">{{ contentRepoStorage.total_space }}</span>
                  </div>
                </div>
              </div>
              ...
              <div v-if="flowFileRepoStorage" class="storage-subsection">
                ...
                <div class="detail-grid">
                  <div class="detail-item">
                    <span class="detail-key">Utilization</span>
                    <span class="detail-value">{{ flowFileRepoStorage.utilization }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Used Space</span>
                    <span class="detail-value">{{ flowFileRepoStorage.used_space }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Free Space</span>
                    <span class="detail-value">{{ flowFileRepoStorage.free_space }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Total Space</span>
                    <span class="detail-value">{{ flowFileRepoStorage.total_space }}</span>
                  </div>
                </div>
              </div>
              ...
              <div v-if="provenanceRepoStorage" class="storage-subsection">
                ...
                <div class="detail-grid">
                  <div class="detail-item">
                    <span class="detail-key">Identifier</span>
                    <span class="detail-value">{{ provenanceRepoStorage.identifier }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Utilization</span>
                    <span class="detail-value">{{ provenanceRepoStorage.utilization }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Used Space</span>
                    <span class="detail-value">{{ provenanceRepoStorage.used_space }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Free Space</span>
                    <span class="detail-value">{{ provenanceRepoStorage.free_space }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Total Space</span>
                    <span class="detail-value">{{ provenanceRepoStorage.total_space }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="detail-section" v-if="garbageCollection && garbageCollection.length > 0">
              <h6 class="section-title">
                <i class="pe-7s-refresh-2"></i> Garbage Collection
              </h6>
              <div v-for="(gc, index) in garbageCollection" :key="index" class="storage-subsection">
                <div class="subsection-title">{{ gc.name }}</div>
                <div class="detail-grid">
                  <div class="detail-item">
                    <span class="detail-key">Collection Count</span>
                    <span class="detail-value">{{ gc.collection_count }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Collection Time</span>
                    <span class="detail-value">{{ gc.collection_time }}</span>
                  </div>
                  <div class="detail-item">
                    <span class="detail-key">Collection Millis</span>
                    <span class="detail-value">{{ gc.collection_millis }} ms</span>
                  </div>
                </div>
              </div>
            </div>
...
// Load all NiFi instances
const loadInstances = async () => {
  try {
    const response = await apiRequest<NiFiInstance[]>('/api/nifi-instances/');
    instances.value = response;
  } catch (err: any) {
    error.value = 'Failed to load NiFi instances: ' + (err.message || 'Unknown error');
  }
};

// Load data for selected instance
const loadInstanceData = async () => {
  ...
  try {
    const response = await apiRequest<any>(
      `/api/nifi-instances/${selectedInstanceId.value}/get-system-diagnostics`
    );
    systemDiagnostics.value = response.data;
  } catch (err: any) {
    error.value = 'Failed to load system diagnostics: ' + (err.message || 'Unknown error');
  } finally {
    loading.value = false;
  }
};
...
const heapUtilization = computed(() => {
  return systemDiagnostics.value?.system_diagnostics?.aggregate_snapshot?.heap_utilization || '0%';
});
...
const cpuLoad = computed(() => {
  ...
  return load !== undefined ? load.toFixed(2) : 'N/A';
});
...
const contentStorageUtilization = computed(() => {
  const repos = systemDiagnostics.value?.system_diagnostics?.aggregate_snapshot?.content_repository_storage_usage;
  if (repos && repos.length > 0) {
    return repos[0].utilization || '0%';
  }
  return '0%';
});
