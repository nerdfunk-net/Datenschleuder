...
echo "🚀 Building Datenschleuder All-in-One Image for Air-Gap Deployment"
echo "=================================================================="
...
OUTPUT_FILE="docs/docker/airgap-artifacts/${IMAGE_NAME}-${IMAGE_TAG}.tar"

# Create output directory
mkdir -p docs/docker/airgap-artifacts
...
docker build -t "${FULL_IMAGE_NAME}" -f docs/docker/Dockerfile.all-in-one . \
    --no-cache ${PROXY_ARGS}
...
echo "4. Run: docker run -d --name datenschleuder \\"
echo "        -p 3000:3000 -p 8000:8000 \\"
echo "        -v datenschleuder-data:/app/data \\"
