...
async def oidc_callback(
    ...
    logger.debug(
        f"[OIDC Debug] Authorization code length: {len(callback_data.code) if callback_data.code else 0}"
    )
    ...
    try:
        ...
        if callback_data.state:
            ...
            if len(state_parts) != 2:
                logger.error(
                    f"[OIDC Debug] Invalid state format: {callback_data.state}"
                )
                raise HTTPException(
            ...
            if state_provider != provider_id:
                logger.error(
                    f"[OIDC Debug] State provider mismatch: expected '{provider_id}', got '{state_provider}'"
                )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            logger.debug("[OIDC Debug] State validation successful")

        # Exchange code for tokens
        logger.debug("[OIDC Debug] Exchanging authorization code for tokens...")
        ...
        if "id_token" not in tokens:
            logger.error(
                f"[OIDC Debug] No ID token in response from provider '{provider_id}'"
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        logger.debug("[OIDC Debug] Verifying ID token...")
        ...
        if not user.is_active:
            logger.warning(
                f"[OIDC Debug] Inactive user '{user.username}' attempted login via OIDC provider '{provider_id}'"
            )
            raise HTTPException(
        ...
        logger.error(
            f"[OIDC Debug] OIDC callback failed for provider '{provider_id}': {e}"
        )
...
async def oidc_logout(
    provider_id: str,
    id_token_hint: str = Query(None, description="Optional ID token hint for logout"),
) -> Dict[str, Any]:
    """
...
async def test_frontend_oidc(request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if supported_types and response_type not in supported_types:
            steps.append(
                f"⚠ Warning: Response type '{response_type}' may not be supported"
            )
        ...
        if supported_scopes:
            unsupported_scopes = [
                s for s in requested_scopes if s not in supported_scopes
            ]
            if unsupported_scopes:
                steps.append(
                    f"⚠ Warning: Scopes may not be supported: {', '.join(unsupported_scopes)}"
                )
            else:
                steps.append("✓ All requested scopes are supported")
