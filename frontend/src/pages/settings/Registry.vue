<template>
  <div class="registry-flows">
        ...
        <table class="table flows-table">
          ...
          <tbody>
            <tr v-for="flow in registryFlows" :key="flow.id">
              <td><strong>{{ flow.flow_name }}</strong></td>
              <td>{{ flow.nifi_instance_name }}</td>
              <td>{{ flow.registry_name }}</td>
              <td>{{ flow.bucket_name }}</td>
              <td class="text-muted small">{{ flow.flow_description || 'No description' }}</td>
              <td class="text-end">
                <b-button size="sm" variant="outline-danger" @click="handleDelete(flow)" title="Remove">
    ...
    <!-- Add Flow Modal -->
    <b-modal v-model="showAddFlowModal" title="Add Registry Flows" size="xl" modal-class="add-flow-modal">
      <div class="add-flow-content">
        ...
        <div class="selection-step">
          ...
          >
            <template #first>
              <b-form-select-option :value="null" disabled>-- Select NiFi Instance --</b-form-select-option>
            </template>
          </b-form-select>
        </div>
        ...
        <div v-if="selectedInstance" class="selection-step">
          ...
          >
            <template #first>
              <b-form-select-option :value="null" disabled>-- Select Bucket --</b-form-select-option>
            </template>
          </b-form-select>
        </div>
        ...
        <div v-if="selectedBucket" class="selection-step">
          ...
          <div v-else class="flows-list">
            <div v-if="availableFlows.length === 0" class="text-muted text-center py-3">
              No flows found in this bucket
            </div>
            <div v-for="flow in availableFlows" :key="flow.identifier" class="flow-item">
              <b-form-checkbox v-model="selectedFlows" :value="flow">
                <div class="flow-info">
                  <div class="flow-name">{{ flow.name }}</div>
                  <div class="flow-desc">{{ flow.description || 'No description' }}</div>
                </div>
              </b-form-checkbox>
            </div>
          </div>
      ...
      <template #footer>
        <b-button variant="secondary" @click="closeModal" size="sm">Cancel</b-button>
        <b-button
          variant="primary"
          @click="saveFlows"
          size="sm"
...
const onInstanceChange = async () => {
  ...
  try {
    ...
    const response = await apiRequest(`/api/nifi-instances/${selectedInstance.value.id}/get-registries`)
    ...
    if (response.registry_clients && response.registry_clients.length > 0) {
      ...
      const bucketsResponse = await apiRequest(`/api/nifi-instances/${selectedInstance.value.id}/registry/${firstRegistry.id}/get-buckets`)
...
const onBucketChange = async () => {
  if (!selectedBucket.value || !selectedInstance.value || !selectedRegistry.value) return
...
const saveFlows = async () => {
  if (!selectedInstance.value || !selectedRegistry.value || !selectedBucket.value || selectedFlows.value.length === 0) {
    return
  }
  ...
  try {
    ...
    alert(`Successfully saved ${result.created} flow(s)` + (result.skipped > 0 ? ` (${result.skipped} duplicate(s) skipped)` : ''))
    closeModal()
