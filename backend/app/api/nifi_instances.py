...
@router.get("/{instance_id}/get-parameters", response_model=ParameterContextListResponse)
        ...
        if hasattr(param_contexts_entity, 'parameter_contexts') and param_contexts_entity.parameter_contexts:
            for context in param_contexts_entity.parameter_contexts:
                ...
                if hasattr(context, 'component') and hasattr(context.component, 'parameters'):
                    for param in context.component.parameters:
                        param_data = ParameterEntity(
                            name=param.parameter.name if hasattr(param, 'parameter') and hasattr(param.parameter, 'name') else "Unknown",
                            description=param.parameter.description if hasattr(param, 'parameter') and hasattr(param.parameter, 'description') else None,
                            sensitive=param.parameter.sensitive if hasattr(param, 'parameter') and hasattr(param.parameter, 'sensitive') else False,
                            value=param.parameter.value if hasattr(param, 'parameter') and hasattr(param.parameter, 'value') and not param.parameter.sensitive else None,
                            provided=param.parameter.provided if hasattr(param, 'parameter') and hasattr(param.parameter, 'provided') else False,
                            referenced_attributes=param.parameter.referenced_attributes if hasattr(param, 'parameter') and hasattr(param.parameter, 'referenced_attributes') else None,
                ...
                context_data = ParameterContext(
                    ...
                    name=context.component.name if hasattr(context, 'component') and hasattr(context.component, 'name') else "Unknown",
                    description=context.component.description if hasattr(context, 'component') and hasattr(context.component, 'description') else None,
                    parameters=parameters,
                    bound_process_groups=bound_groups if bound_groups else None,
                    inherited_parameter_contexts=inherited_contexts if inherited_contexts else None,
                    component_revision=context.revision.to_dict() if hasattr(context, 'revision') and hasattr(context.revision, 'to_dict') else None,
                    permissions=context.permissions.to_dict() if hasattr(context, 'permissions') and hasattr(context.permissions, 'to_dict') else None,
                )
    ...
    try:
        ...
        return {
            ...
            "parameter_context": {
                ...
                "name": result.component.name if hasattr(result, 'component') else data.name,
            }
        }
...
async def update_parameter_context(
    ...
    try:
        ...
        print(f"Existing context has {len(existing_context.component.parameters) if existing_context.component.parameters else 0} parameters")
        print(f"Update data has {len(data.parameters) if data.parameters else 0} parameters")
        ...
        if data.parameters is not None:
            ...
            for param in data.parameters:
                ...
                if param.name in existing_param_map:
                    ...
                    if not param.sensitive:  # Only update value for non-sensitive parameters
            ...
            for param_name in existing_param_map.keys():
                if param_name not in desired_param_names:
                    ...
                    delete_param_entity = NiFiParameterEntity(parameter=delete_param_dto)
        ...
        print(f"Final parameters list has {len(parameters)} parameters (including deletions)")
        for p in parameters:
        ...
        while attempt < max_attempts:
            ...
            percent = status_response.request.percent_completed if hasattr(status_response.request, 'percent_completed') else 0
            state = status_response.request.state if hasattr(status_response.request, 'state') else 'unknown'
            print(f"Attempt {attempt}: complete={status_response.request.complete}, state={state}, percent={percent}%")
            ...
            if hasattr(status_response.request, 'update_steps'):
                ...
                for step in status_response.request.update_steps:
                    if hasattr(step, 'description') and hasattr(step, 'complete'):
                        print(f"    - {step.description}: {'complete' if step.complete else 'pending'}")
            ...
            if status_response.request.complete:
                ...
                if hasattr(status_response.request, 'failure_reason') and status_response.request.failure_reason:
                    ...
                    raise Exception(f"Update failed: {status_response.request.failure_reason}")
                ...
                actual_param_count = len(updated_context.component.parameters) if updated_context.component.parameters else 0
                expected_param_count = len(parameters)

                print(f"Expected {expected_param_count} parameters, got {actual_param_count}")
                if updated_context.component.parameters:
                    for p in updated_context.component.parameters:
                        print(f"  - {p.parameter.name if hasattr(p, 'parameter') else 'unknown'}")
